...
for i in range(1):
    ...
    a1,b1,c1 = np.linalg.solve(F[inds, :], t[inds])
    a2,b2,c2 = np.linalg.lstsq(F, t)[0]
    
    Face_rnd = a1 * Face1 + b1 * Face2 + c1 * Face3
    Face_lsq = a2 * Face1 + b2 * Face2 + c2 * Face3

    # plot_face(plt, NoisyTargetFace, edges, color='g')
    plot_face(plt, Face_rnd, edges, color='r')
    plot_face(plt, TargetFace, edges, color='g')
    plot_face(plt, Face_lsq, edges, color='b')

    print('error=', np.linalg.norm(TargetFace - Face_lsq))
    print('error=', np.linalg.norm(TargetFace - NoisyTargetFace))
